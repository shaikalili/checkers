...
var haveToEat;
//var checkers = checkers.slice();
var optionalEnding=["the game is tie","white player win","black player win"];
initCheckers();
...
function endGame(){
  if(isWhitePlyerWin()&&isblackPlyerWin()){return 0;}
  else if( isWhitePlyerWin()){ return 1;}
  else if( isblackPlyerWin()){return 2;}
  else {return false;}
}
...
function moveSelectedCheckerHere() {
  ...
  if ((checkers[targetId].color.includes("white")&&checkers[current].color.includes("white"))||
  (checkers[targetId].color.includes("black")&&checkers[current].color.includes("black"))) {
   $(".turn-" + activplayer).addClass("active");
    $(`.pressed`).removeClass(`pressed`);
...
function changeTurn(targetId){
  ...
  if(endGame()){
    $(".turn-" + activplayer).html(optionalEnding[endGame()]);
    $(".turn-" + activplayer).addClass("win");
  }
 else{ checkers[targetId].color === "black"||(checkers[targetId].color==="black-king")
  ? (activplayer = 0)
  : (activplayer = 1);
$(".turn-" + activplayer).addClass("active");
